    ...
    if (xhr.readyState === 4) {
        if (xhr.status === 200) {
            ...
            colorSelect.addEventListener("click", (event) => {
                ...
                const variantChoose = [];
                for (const i in data1.variants) {
                    if (color === hexToRGB("#" + data1.variants[i].color_code)) {
                        variantChoose.push(data1.variants[i]);
                    }
                }
                ...
                for (const i in variantChoose) { // disable size if there is no more stock
                    if (variantChoose[i].stock === 0) {
                        ...
                        for (const j in sizeDisable) {
                            if (variantChoose[i].size === sizeDisable[i].innerHTML) {
                                sizeDisable[i].className = "product__size product__size--disabled";
                            }
                        }
                    }
                }
...
xhr.open("GET", `http://localhost:3000/api/1.0/products/details?id=${id}`); // for test

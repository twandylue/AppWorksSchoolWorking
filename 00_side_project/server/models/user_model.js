...
const getRecord = async (email) => {
    const conn = await pool.getConnection();
    const results = await conn.query("SELECT player_email, SUM(points), COUNT(*) FROM game_history WHERE player_email = ?", email);
    const totalPoints = parseInt(results[0][0]["SUM(points)"]);
    const hitRate = (totalPoints * 2 / 10) / parseInt(results[0][0]["COUNT(*)"]);

    const sql = "SELECT game_results.game_id, user.name, user.email, game_setting_info.room_id, game_setting_info.type, game_setting_info.number, game_setting_info.rounds, game_setting_info.player_1, game_setting_info.player_2, game_results.round1_points, game_results.round2_points, game_results.round3_points, game_results.total_points, game_results.hit_rate, game_results.winner_email FROM game_results INNER JOIN `user` ON user.email = game_results.player_email INNER JOIN game_setting_info ON game_setting_info.id = game_results.game_id WHERE game_results.player_email = ? ;";
...
const getLeaderList = async () => {
    ...
    const conn = await pool.getConnection();
    const results = await conn.query("SELECT user.name, player_email, SUM(total_points), SUM(hit_rate), COUNT(*) FROM game_results INNER JOIN user ON user.email = game_results.player_email GROUP BY player_email ORDER BY SUM(total_points) DESC;");
    await conn.release();

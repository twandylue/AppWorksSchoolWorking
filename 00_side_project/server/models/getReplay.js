...
const getReplay = async (gameID) => {
    ...
    try {
        ...
        if (status[0][0].status === 1) { // 或許只提供真人對戰重播 待改
            ...
            members = await conn.query("SELECT user.name, game_results.player_email FROM game_results INNER JOIN user ON user.email = game_results.player_email WHERE game_id = ?", [gameID]);
